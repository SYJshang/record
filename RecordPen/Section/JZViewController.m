...
- (void)AVAssetExportMP4SessionStatusFailed:(id)object
{
    ...
    if (self.videoInputURL && [self.videoInputURL isFileURL])
    {
        ...
        NSString *defaultVideo = @"Demo1.mp4";
    ...
    [blockSelf setCallbackPickMusic:^(BOOL success, id result)
     {
         if (success)
         {
             NSArray *musics = [NSArray arrayWithObjects:@"Big Big World", @"Yesterday Once More", @"The Day You Went Away",  @"Because I Love You", nil];
...
{
    CGFloat orginHeight = self.view.frame.size.height - 134;
    if (iOS6 || iOS5)
    {
        orginHeight += 20;
    }
...
- (void)viewDidLoad
{
    ...
    {
        // Preview demo video
        NSString *demoFile = [NSString stringWithFormat:@"Demo1.mp4"];
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    self.navigationItem.titleView = [UILabel titleWithColor:[UIColor colorWithWhite:0.5 alpha:1.0] title:@"Video editing" font:17.0];
