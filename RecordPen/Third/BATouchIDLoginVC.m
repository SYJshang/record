...
- (void)test
{
     ...
     */
    [BAKit_TouchID ba_touchIDWithContent:@"This operation requires authentication of your identity" cancelButtonTitle:nil otherButtonTitle:nil enabled:YES BAKit_TouchIDTypeBlock:^(BAKit_TouchIDType touchIDType, NSError *error, NSString *errorMessage) {
        ...
        {
            BAKit_ShowAlertWithMsg(@"Fingerprint verification success！");
...
- (void)ba_backToPreVC:(CLLockVC *)lockVC
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.isStart)
        {
//            SYJNavitionController *navi = [[SYJNavitionController alloc] initWithRootViewController:[ZYTabBarController new]];
            
            [self presentViewController:[ZYTabBarController new] animated:NO completion:nil];
...
- (UIButton *)touchIDButton
{
    if (!_touchIDButton)
    {
        ...
        [_touchIDButton setTitle:@"Click to unlock the fingerprint" forState:UIControlStateNormal];
