...
- (void)test
{
    ...
    {
        [_dataArray addObjectsFromArray:@[@"Gestures password", @"Verify gesture password", @"Change the password"]];
    }
    else if ([isOpenUnlockedGesture intValue] == 0 && _dataArray.count == 0)
    {
        [_dataArray addObject:@"Gestures password"];
    }
...
{
    [UIAlertController ba_alertControllerShowAlertInViewController:self withTitle:nil mutableAttributedTitle:nil message:@"Continue to unlock the gesture unlock \n will close the fingerprint unlock" mutableAttributedMessage:nil buttonTitlesArray:@[@"cancel", @"continued"] buttonTitleColorArray:@[[UIColor greenColor], [UIColor redColor]] tapBlock:^(UIAlertController * _Nonnull controller, UIAlertAction * _Nonnull action, NSInteger buttonIndex) {
