...
- (void)event
{
    ...
    self.lockView.setPWSErrorLengthTooShortBlock = ^(NSUInteger currentCount){
        ...
        [self.label showWarnMsg:[NSString stringWithFormat:@"Please connect at least %@ point",@(CoreLockMinItemCount)]];
    };
...
//        }
    ...
    self.lockView.verifyPwdBlock = ^(NSString *pwd){
        ...
        else
        {
            ...
            self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Forgot password" style:UIBarButtonItemStylePlain target:self action:@selector(forgetPwd)];;
        }
        return res;
    };
                        ...
                        successBlock:(void(^)(CLLockVC *lockVC, NSString *pwd))successBlock
{
    ...
    lockVC.title = @"Gestures to unlock";
                        ...
                        successBlock:(void(^)(CLLockVC *lockVC, NSString *pwd))successBlock
{
    ...
    lockVC.title = @"Change the password";
    //设置类型
...
- (UIBarButtonItem *)resetItem
{
    ...
    {
        //添加右按钮
        _resetItem= [[UIBarButtonItem alloc] initWithTitle:@"reset" style:UIBarButtonItemStylePlain target:self action:@selector(setPwdReset)];
    }
